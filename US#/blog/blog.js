...
const getObject = async url => {
    ...
    const obj = await response.json(); // Await the response.json()
    return obj;
};
...
$(document).ready(async () => {
    try {
        const post_ids = [2, 12, 22, 32, 42];
        // get first 5 blog posts
        const post_promises = post_ids.map(postId => getObject(`${domain}/posts/${postId}`));
        const posts = await Promise.all(post_promises);
        // get user for each post
        const user_promises = posts.map(
            post => getObject(`${domain}/users/${post.userId}`)
        );
        const users = await Promise.allSettled(user_promises);
